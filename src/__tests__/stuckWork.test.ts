...
describe('reading the queue', () => {
  it('says nothing about a queue that is simply working', () => {
    const s = assessQueue([item(), item(), item({ status: 'sending' })], { pending: 2, unknown: 0, failed: 0 }, NOW);
    expect(s.clear).toBe(true);
    expect(stuckWords(s)).toBeUndefined();
  });

  it('says nothing about an empty queue', () => {
    expect(assessQueue([], undefined, NOW).clear).toBe(true);
  });

  it('counts a refusal straight away, however new it is', () => {
    // A refusal is not going to fix itself by waiting.
    const s = assessQueue([item({ status: 'failed', createdAt: ago(1000) })], undefined, NOW);
  ...
  it('counts a send that got no reply separately from a refusal', () => {
    ...
    const s = assessQueue([item({ status: 'unknown' })], undefined, NOW);
  ...
  it('leaves a pending item alone until it has been a day', () => {
    const nearly = assessQueue([item({ createdAt: ago(STALE_AFTER_MS - 60_000) })], undefined, NOW);
    expect(nearly.waiting).toBe(0);
    const over = assessQueue([item({ createdAt: ago(STALE_AFTER_MS + 60_000) })], undefined, NOW);
  ...
  it('treats a date it cannot read as stuck rather than ignoring it', () => {
    ...
    const s = assessQueue([item({ createdAt: 'not a date' })], undefined, NOW);
    expect(s.waiting).toBe(1);
  });

  it('counts photographs that will not upload alongside notes that will not send', () => {
    const s = assessQueue([], { pending: 12, unknown: 1, failed: 3 }, NOW);
...
describe('what it says', () => {
  ...
  it('is a warning, not a failure, when things are only waiting', () => {
    const w = stuckWords(assessQueue([item({ createdAt: ago(3 * STALE_AFTER_MS) })], undefined, NOW));
  ...
  it('counts in words a person uses, and gets the singular right', () => {
    const one = stuckWords(assessQueue([item({ status: 'failed' })], undefined, NOW));
    expect(one?.body).toContain('1 thing the office refused');
    const two = stuckWords(assessQueue([item({ status: 'failed' }), item({ status: 'failed' })], undefined, NOW));
    expect(two?.body).toContain('2 things the office refused');
  });

  it('says when the oldest has been sitting, in Queensland time', () => {
    const w = stuckWords(assessQueue([item({ status: 'failed', createdAt: '2026-07-07T23:30:00.000Z' })], undefined, NOW));
