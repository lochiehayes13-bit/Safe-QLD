...
export function pitotFlow(input: PitotFlowInput): PitotFlow | Refused {
  ...
  const issues: Issue[] = [];
  // The gate is `<` and not `> 0 &&` deliberately. A dead-zero pitot returns a
  // flow of zero, which is a number a report would print — and it is far more
  // often a tube out of the stream or a gauge left shut than a hydrant that
  // genuinely delivers nothing. It gets the same warning as any other unusable
  // reading rather than passing silently.
  if (pitotKpa < PITOT_MIN_RELIABLE_KPA) {
    issues.push({
      level: 'warning',
...
/**
 * A C value above this is a typing error, not a conduit.
 *
 * Refusing it matters in one direction only: a too-high C understates the
 * friction, and an understated friction is what gets a marginal riser signed
 * off. There is no material with a C near it.
 */
export const HW_C_IMPLAUSIBLE_ABOVE = 200;
...
export function frictionLoss(input: FrictionLossInput): FrictionLoss | Refused {
  ...
  if (input.cOverride !== undefined) {
    ...
    // An upper bound, because the error this catches is always the same one: a
    // slipped decimal point. C = 1400 for C = 140 cuts the estimated loss by
    // about 99%, and a friction figure that small turns a marginal riser into a
    // comfortable one on paper. Nothing water flows through has a C anywhere
  ...
  // The rate is computed first and the total derived from it, rather than the
  // other way round. Hazen-Williams is exactly linear in length, so the two are
  // the same arithmetic — but dividing the total by the length reports 0 kPa/m
  // for a zero-length run, and a hose does not stop having a loss rate because
  // nobody has said how much of it is laid out yet.
  const headLossPerMetreM =
    (HAZEN_WILLIAMS_SI_CONSTANT * Math.pow(qM3s, 1.852)) / (Math.pow(c, 1.852) * Math.pow(dM, 4.87));
  const headLossM = headLossPerMetreM * lengthM;
  ...
  return {
    ...
    lossKpaPerM: round(headLossPerMetreM * KPA_PER_METRE_OF_HEAD, 4),
...
export interface RequirementRef {
  ...
  /**
   * Which state of the system a ceiling applies to, because they are different
   * numbers: the Queensland guidance permits 1300 kPa static with the pump
   * running and 1200 kPa at the outlet under design flow. 'unstated' is a real
   * answer and is used where the source does not say — checking a static
   * against a figure written for the flowing case invents a defect.
   */
  appliesAt?: 'no-flow' | 'design-flow' | 'unstated';
  note?: string;
}
...
export interface AssessmentInput {
  ...
  staticKpa?: number;
  /**
   * Maximum permitted pressure at the outlet **while flowing**, kPa, where the
   * installation has one. Checked against the measured residual and nothing
   * else — see maxStaticKpa.
   */
  maxOutletKpa?: number;
  /**
   * Maximum permitted pressure at the outlet **at no flow**, kPa.
   *
   * A separate field on purpose. The Queensland guidance shipped in
   * REQUIREMENT_REFS sets 1200 kPa at the outlet under design flow and 1300 kPa
   * static with the pump running: two figures for two states of the system.
   * Checking a static against the flowing ceiling manufactures a defect that
   * document does not support, so neither ceiling is inferred from the other.
   */
  maxStaticKpa?: number;
 ...
 * hold the hose — and it stays a defect where the flow question came out
 * indeterminate, because it was measured rather than projected.
 *
 * There are two ceilings and they are not interchangeable. The pressure allowed
 * at an outlet while it is flowing is a lower figure than the static allowed at
 * no flow — the Queensland guidance in REQUIREMENT_REFS says 1200 and 1300 kPa
 * respectively — so each is checked against its own reading or it is not
 * checked at all. Failing a static against the flowing figure writes up a
 * defect the source does not support, which is the same false statement as
 * missing a real one, pointing the other way.
 */
export function assessHydrant(input: AssessmentInput): Assessment | Refused {
  ...
  // optional reading below arrives off a text field: left as NaN, a ceiling is
  // dropped and the over-pressure check is skipped in silence, and the result
  // reads PASS to a technician who believes the maximum was checked.
  const unreadable = (
    [
      ['static pressure', input.staticKpa],
      ['maximum flowing pressure', input.maxOutletKpa],
      ['maximum static pressure', input.maxStaticKpa],
    ] as const
  ...
  // Over-pressure is worked out first and applied last: it is a demonstrated
  // defect, so it fails the assessment whatever the flow question turned out to
  // be — including when the flow question could not be answered at all.
  let overPressure = false;
  if (input.maxOutletKpa !== undefined && measuredResidualKpa > input.maxOutletKpa) {
    overPressure = true;
    issues.push({
      level: 'error',
  ...
  // limit is a different and higher figure. Saying so is the difference between
  // a warning and a defect notice nobody can support.
  if (
    input.maxOutletKpa !== undefined &&
    input.maxStaticKpa === undefined &&
    input.staticKpa !== undefined &&
    input.staticKpa > input.maxOutletKpa
  ) {
    issues.push({
      level: 'warning',
  ...
  // Applied here rather than inside each branch, because an outlet over its
  // ceiling was measured, not inferred. It is a defect even where the flow
  // question came out indeterminate — an incomplete flow test does not make a
